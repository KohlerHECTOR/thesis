...
In this section we present decision tree policies of this manuscript obtained using Dagger or VIPER~\citep{dagger,viper} after learning an expert Q-function for the grid world MDP.
...
While for Q-learning, Sutton and Barto break ties by index value in their book~\citep{sutton} (the greedy action is the $\operatorname{argmax}$ action with smallest index), we show that the choice of tie-breaking greatly influences the performance of subsequent imitation learning algorithms.
...
\begin{algorithm}
    ...
    \Fn{\SubtreeFromPolicy{$\boldsymbol{o}, \pi_{po}$}}{
        ...
        \Else{
            $\langle i, v\rangle \leftarrow a;\quad \boldsymbol{o}_L \leftarrow \boldsymbol{o}; \quad \boldsymbol{o}_R \leftarrow \boldsymbol{o}$ \\
            ...
            $child_L \leftarrow$ Subtree\_From\_Policy$(\boldsymbol{o}_L, \pi_{po}); \ child_R \leftarrow$ Subtree\_From\_Policy$(\boldsymbol{o}_R, \pi_{po})$ \\
...
We attempt to reproduce the results from~\cite{topin2021iterative} in which authors compare direct and indirect learning of decision tree policies of depth at most 2 for the CartPole MDP~\citep{cartpole}.
...
Authors of~\cite{topin2021iterative} use two deep reinforcement learning baselines to which they apply some modifications in order to learn memoryless policies.
Authors modify the standard DQN~\citep{dqn} to return a memoryless policy. The trained $Q$-function is approximated with a neural network $O\rightarrow \mathbb{R}^{|A\cup A_{info}|}$ rather than $S\times O\rightarrow \mathbb{R}^{|A\cup A_{info}|}$.
...
Similar modifications are applied to the standard PPO~\citep{ppo} that we present in the appendix (algorithm \ref{alg:mod-ppo}). In the modified PPO, neural network policy $O\rightarrow A\cup A_{info}$ is trained using a neural network value function $S\times O\rightarrow A\cup A_{info}$ as a critic.

Those two variants of DQN and PPO have first been introduced in \citep{pinto} for robotic tasks with partially observable components, under the name ``asymmetric'' actor-critic. 
Asymmetric RL algorithms that have policy and value estimates using different information from a POMDP~\citep{POMDP,chap2} were later studied theoretically to solve POMDPs in Baisero's work~\citep{baisero-dqn,baisero-ppo}.
The connections from Deep RL in IBMDPs for objective is absent from \citep{topin2021iterative} and we defer their connections to direct interpretable reinforcement learning to the next chapter as our primary goal is to reproduce \citep{topin2021iterative} \textit{as is}.
Next, we present the precise experimental setup we use to reproduce~\citep{topin2021iterative} in order to study direct deep reinforcement learning of decision tree policies for the CartPole MDP.
...
Authors of~\citep{topin2021iterative} propose to parametrize the set of IGAs with $j \times q$ actions $\langle j, v_k \rangle$ with $v_k$ depending on the current observation $\boldsymbol{o}_t=(L'_1, U'_1, \dots, L'_j, U'_j, \dots, L'_p, U'_p)$: $v_k = \frac{k(U'_j - L'_j)}{q+1}$.
...
We use the exact same downstream MDP and associated IBMDPs for our experiments as~\citep{topin2021iterative} except when mentioned otherwise.

\paragraph{Downstream MDP} The task at hand is to optimize the RL objective (definition~\ref{def:mdp-obj}) with a decision tree policy for the CartPole MDP~\citep{cartpole}.
...
Like in~\citep{topin2021iterative}, we use the gymnasium \texttt{CartPole-v0} implementation~\citep{gymnasium} of the CartPole MDP in which trajectories are truncated after 200 timesteps making the maximum cumulative reward, i.e. the optimal value of the RL objective when $\gamma=1$, to be 200.
...
\begin{algorithm}
    ...
    \caption{Modified Deep Q-Network. We highlight in green the changes to the standard DQN~\citep{dqn}.}\label{alg:mod-dqn}
\end{algorithm}
...
Authors did not share which DQN implementation they used so we use the stable-baselines3 one~\citep{stable-baselines3}\footnote{We are cleaning our source code and will open source it as soon as possible}.
...
For each indirect method, we imitate the neural network experts by fitting decision trees on 10000 expert transitions using the CART~\citep{breiman1984classification} implementation from scikit-learn~\citep{scikit-learn} with default hyperparameters and maximum depth of 2 like in ~\citep{topin2021iterative}.
...
Next, we present our results when reproducing~\citep{topin2021iterative}.
...
\begin{figure}
    ...
    }\label{fig:dt-distrib-poibmdp-each-zeta}
\end{figure}
...
\begin{algorithm}
    ...
    $\pi(\boldsymbol{o}) = \operatorname{argmax}_a Q(\boldsymbol{o},a)$ \Comment{// Extract greedy policy}
...
\small
\centering
\caption{SARSA-Asym Hyperparameter Space (768 combinations)}\label{tab:sarsa-asym-hp}
\begin{tabular}{lll}
\toprule
\textbf{Hyperparameter} & \textbf{Values} & \textbf{Description} \\
\midrule
