...
\begin{algorithm}
    \KwData{IBMDP $\mathcal{M}_{IB}\langle S\times O, A\cup A_{\info}, (R, \zeta),( T, T_0, T_{info})\rangle$, learning rate $\alpha$, exploration rate $\epsilon$, partially observable Q-network parameters $\theta$, Fully observable Q-network parameters $\phi$, replay buffer $\mathcal{B}$, update frequency $C$}
    \KwResult{Partially pbservable deterministic policy $\pi_{po}$}
    Initialize partially observable Q-network parameters $\theta$\\
    Initialize fully observable Q-network parameters $\phi$ and target network parameters $\phi^- = \phi$ \\
    ...
    \For{each episode}{
        ...
        \For{each step $t$}{
            Choose action $a_t$ using $\epsilon$-greedy: $a_t = \arg\max_a Q_\theta(o_t,a)$ with prob. $1-\epsilon$ \\
            ...
            $a' = \underset{a}{\operatorname{argmax}} Q_{\theta}(o_i, a)$ \\
            $y_i = r_i + \gamma Q_{\phi^-}(s_{i+1}, a')$ \Comment{// Compute target}
            $\phi \leftarrow \phi - \alpha \nabla_\phi (Q_\phi(s_i, a_i) - y_i)^2$ \Comment{// Update fully observable Q-network}
            $\theta \leftarrow \theta - \alpha \nabla_\theta (Q_\theta(o_i, a_i) - y_i)^2$ \Comment{// Update partially observable Q-network}
    ...
    $\pi_{po}(o) = \arg\max_a Q_\theta(o,a)$ \Comment{// Extract greedy policy}
...
\begin{algorithm}
    ...
    \For{each episode}{
        Generate trajectory $\tau = (s_0, o_0, a_0, r_0, s_1, o_1, a_1, r_1, \ldots)$ following $\pi_\theta$ \\
        \For{each timestep $t$ in trajectory}{
            ...
            $r_t(\theta) \leftarrow \frac{\pi_{po_\theta}(a_t|o_t)}{\pi_{po_\theta}_{old}(a_t|o_t)}$ \Comment{// Compute probability ratio}
