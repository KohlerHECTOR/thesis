...
\begin{figure}
    ...
    Shaded areas represent the standard deviation at each measure on the y-axis.}
\end{figure}
On Figures (cite) and (cite) we observe that learning curves for Modified DQN and Modified PPO on IBMDPs can learn--the curves have an increasing trend--but find poor decision tree policies for CartPole in average--the curves flatten at the end of the x-axis and have low y-values--.
...
On the other hand, on Figures (cite) and (cite) we observe that baselines seeking fully observable policies (RL + IBMDP and RL + MDP), learn better CartPole policies and in average. We do notice that for DQN-like baselines, learning seems difficult in general indepedently of the setting while on Figure (cite) it is clear that for PPO baselines seeking fully observable, learning is super efficient and agent find optimal policies with reward 200.

Interestingly, we find that the top-5 Modified DQN baselines + IBMDP in terms of final average CartPole tree policy performances all have $p=2$ and either depth control with the reward or with a termination signal. 
This IBMDP hyperparameters control the available IGAs available to the policy and differs from the original paper's $p=3$ value.
We find a potential explanation as to why $p=2$ could work better than $p=3$ despite giving the agent fewer candidates nodes to consider.
...
\begin{table}[h]
    ...
    \midrule
    1 & 2 & termination & $\operatorname{tanh}()$ & 0.5 & 0.01 & 79.16 \\
    2 & 2 & termination & $\operatorname{tanh}()$ & 0.5 & -0.01 & 72.92 \\
    3 & 2 & termination & $\operatorname{tanh}()$ & 0.9 & 0.01 & 63.85 \\
    4 & 2 & termination & $\operatorname{tanh}()$ & 0.9 & -0.01 & 54.36 \\
    5 & 3 & termination & $\operatorname{tanh}()$ & 0.5 & -0.01 & 27.30 \\
    \bottomrule
    \end{tabular}
    \end{table}
    ...
    \begin{table}[h]
        ...
        \midrule
        1 & 3 & none & $\operatorname{tanh}()$ & -0.01 & 89.24 \\
        2 & 3 & reward & $\operatorname{relu}()$ & 0.01 & 71.73 \\
        3 & 3 & reward & $\operatorname{tanh}()$ & 0.01 & 54.92 \\
        4 & 3 & none & $\operatorname{tanh}()$ & 0.01 & 52.55 \\
        5 & 3 & reward & $\operatorname{relu}()$ & -0.01 & 38.49 \\
        \bottomrule
        \end{tabular}
        \end{table}
