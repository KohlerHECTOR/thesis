...
\begin{algorithm}
    ...
    \For{each episode}{
        ...
        \For{each step $t$}{
            ...
            $\phi \leftarrow \phi - \alpha \nabla_\phi (Q_\phi(\boldsymbol{s}_i, a_i) - y_i)^2$ \Comment{// Update fully observable Q-network}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[leaf] (tree7_root) at (-6,0) {$\rightarrow$};
        \draw[->] (tree7_root) to[out=45,in=0,looseness=5] (tree7_root);

        \node[decision] (tree7_root) at (-6,-4) {$\dot{\theta} \leq 0$};
        \draw[->] (tree7_root) to[out=45,in=0,looseness=5] (tree7_root);
        ...
        % Labels
        \node[above] at (-8,1) {{\small Most frequent modified PPO tree (12)}};
        \node[above] at (-7,-3) {{\small Most frequent modified DQN tree (9.5)}};

        \node[above] at (-1,1) {{\small Best modified PPO tree (175)}};
