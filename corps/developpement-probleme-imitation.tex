...
Because in this part of the manuscript we do not evaluate learning capabilities of interpretable machine learning algorithms but solely their outputs, we use indirect imitation learning (cf. section~\ref{sec:imit}) to obtain policies from different classes.
...
We consider linear policies that have been shown to be able to solve MuJoCo tasks~\cite{random-search}.
We fit linear policies to expert policies using simple linear (logistic) regressions with scikit-learn's~\cite{scikit-learn} default implementation.
We also consider decision trees~\cite{breiman1984classification} and oblique decision trees~\cite{murthy1994system}.
Oblique decision trees are decision trees which do not necessarily partition the input domain parallel to axes.
...
\subsection{Which expert policies to imitate with which algorithm?}
...
We do not use PPO experts with VIPER either: despite working with discrete actions PPO does not compute a $Q$-function necessary for the samples re-weighting in VIPER.
...
We also specify the total number of samples collected during the imitations as well as the number of times we repeat the imitations for statistical significance.
...
\begin{table}
  ...
  \bottomrule
  \end{tabular}
  \caption{Repetitions of each imitation learning algorithm on each environment. We specify in parentheses from which deep RL algorithms of the zoo~\cite{zoo} expert policies come.}
...
Because there seems to not be a more interpretable policy class across proxy metrics, we will keep studying both metrics at the same time.
...
We obtain diverse baseline policies with diverse interpretability measures and performances for a wide range of environments.
