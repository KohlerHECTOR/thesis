...
In this manuscript, we study algorithms for interpretable sequential decision making. 
...
Consider the slightly extreme scenario of a combat drone in which a program decides at whom to shoot shoot.
...
Such guarantees can be obtained through formal verifications of programs which in turn can only be obtained for certain programs.
...
In the next section, we present related works that fit this desiderata.
...
The classification and regression trees (CART) algorithm \cite{breiman1984classification} (cf. algorithm~\ref{alg:cart}), developed by Leo Breiman and colleagues in 1984, is one of the most widely used methods for learning decision trees from supervised data.
...
At each internal node, the algorithm selects the feature and its threshold that best splits the data according to a purity criterion such as the Gini impurity for classification or mean squared error for regression.
...
\begin{algorithm}
    ...
    \caption{CART~\cite{breiman1984classification}}\label{alg:cart}
\end{algorithm}
...
\begin{definition}[Markov decision process]\label{def:mdp} An MDP is a tuple $\mathcal{M} \equiv \langle S, A, R, T, T_0 \rangle$ where:
\begin{itemize}
...
\item $R: S \times A \rightarrow \mathbb{R}$ is a deterministic reward function that assigns a real-valued reward to each state-action pair. While in general reward functions are often stochastic, in this manuscript we focus on deterministic ones without loss of generality.
...
The well-known value iteration algorithm (cf. algorithm~\ref{alg:value-iteration}), computes the values of states $\V^{\star}(s)$ with respect to the policy $\pi^{\star}$ maximizing the RL objective. 

\RestyleAlgo{ruled}
\SetKwComment{Comment}{}{}
\begin{algorithm}
    \KwData{MDP $\mathcal{M} \equiv \langle S, A, R, T, T_0 \rangle$}
    \KwResult{Optimal values $V^{\star}$ and optimal policy $\pi^*$}
    Initialize $V(s) = 0$ for all $s \in S$ \\
    \Repeat{Convergence}{
        \For{each state $s \in S$}{
            $v \leftarrow V(s)$ \\
            $V(s) \leftarrow \max_a \left[ R(s,a) + \gamma \mathbb{E}\left[V(s')|s'\sim T(s, a)\right]\right]$ \Comment{// Bellman optimality update}
        }
    }
    \For{each state $s \in S$}{
...
\SetKwComment{Comment}{}{}
\begin{algorithm}
    \KwData{MDP $\mathcal{M} \equiv \langle S, A, R, T, T_0 \rangle$, learning rate $\alpha$, exploration rate $\epsilon$}
    \KwResult{Policy $\pi$}
...
\begin{algorithm}
    ...
    $\pi(s) = \operatorname{argmax}_a Q(s,a)$ \Comment{// Extract greedy policy}
    \caption{Sarsa~\cite{sutton}}\label{alg:sarsa}
\end{algorithm}
...
\SetKwComment{Comment}{}{}
\begin{algorithm}
    \KwData{MDP $\mathcal{M} \equiv \langle S, A, R, T, T_0 \rangle$, learning rate $\alpha$, policy parameters $\theta$}
    ...
    \caption{Policy Gradient (REINFORCE algorithm)~\cite{pg_sutton,reinforce}}\label{alg:reinforce}
\end{algorithm}
...
\begin{algorithm}
    \KwData{MDP $\mathcal{M} \equiv \langle S, A, R, T, T_0 \rangle$, learning rate $\alpha$, exploration rate $\epsilon$, Q-network parameters $\theta$, update frequency $C$}
    \KwResult{Policy $\pi$}
    ...
    \caption{Deep Q-Network (DQN)~\cite{dqn}}\label{alg:dqn}
\end{algorithm}


\begin{algorithm}
    \KwData{MDP $\mathcal{M} \equiv \langle S, A, R, T, T_0 \rangle$, learning rate $\alpha$, policy parameters $\theta$, clipping parameter $\epsilon$, value function parameters $\phi$}
    \KwResult{Policy $\pi_\theta$}
...
\begin{algorithm}
    ...
    \KwOut{Fitted student policy $\hat{\pi}'$}
    ...
    \For{$i \gets 1$ \KwTo $N$}{
        ...
        % Train new policy
       Fit classifier/regressor $\hat{\pi}'$ on $\mathcal{D}$\;\label{alg:fit}
    }
    \Return $\hat{\pi}'$\;
    \end{algorithm}
...
Next, we present the outline of thesis.
