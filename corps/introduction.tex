...
Most existing research has focues on developing indirect methods. 
...
\end{figure}
As mentioned earlier, in contrast to neural networks, decision trees are considered highly interpretable because they apply Boolean operations on inputs without relying on internal complex representations.
...
\begin{definition}[Reinforcement learning objective] Given an MDP $M$ (cite) with unknown transitions and rewards, the reinforcement learning objective, is to maximize the expected states values over the initial distribution:
\begin{align}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{images/dqn_schema.png}
