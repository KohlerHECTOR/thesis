...
Such machine learning algorithms train increasingly performant models that are deployed to, e.g., identify digits in images~\cite{lenet}, control tokamak fusion~\cite{tokamak}, or write the abstract of a scientific article~\cite{bert}.
...
Despite this lack of formalism, the necessity of deploying interpretable models has sparked many works, which we present next.
...
The most famous local explanation algorithm is LIME (Local Interpretable Model-agnostic Explanations)~\cite{lime}. Given a program, LIME works by perturbing the input and learning a simple interpretable model locally to explain that particular prediction. For each individual prediction, LIME provides explanations by identifying which features were most important for that specific decision.
Hence, as stated above, LIME needs to learn a local surrogate model per recommendation to be interpreted; this requires substantial computation.
...
In the first part of the manuscript, we show that direct decision tree learning methods struggle to find decision trees even for very simple sequential decision making problems.
...
Finally, after thoroughly studying decision trees and direct methods, we leverage the diversity and simplicity of \textit{indirect} methods to compare other model classes and show that, in some cases, neural networks can be considered more interpretable than trees, and there exist problems for which there is no need to trade off performance for interpretability. 
...
\begin{enumerate}
    ...
    \item In practice, controlling MDPs with interpretable policies does not necessarily decrease performance and has many advantages.
\end{enumerate}
...
\begin{definition}[Factored Markov decision process] A factored MDP is an MDP (cite) where the state space is a hyperrectangle: $S\subseteq \mathbb{R}^n$.
\end{definition}
...
\begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{A grid-world MDP (left) and optimal actions w.r.t. the objective (cite) (right).}
    \end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{images/dqn_schema.png}
...
\begin{algorithm}
    ...
    \begin{algorithm}
        ...
        \For{$i \gets 1$ \KwTo $N$}{
            ...
            Sample transitions from $M$ using $\hat{\pi}$\;
            % Get expert actions for visited states
            Weight each transition by $w(s) \gets V^{\pi^*}(s) - \min_a Q^{\pi^*}(s, a)$\;
...
In the left plot of Figure (cite), we see that Q-learning, independently of how ties are broken, consistently converges to an optimal policy over 100 runs (random seeds).
