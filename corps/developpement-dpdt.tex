...
The set of states is $S = \{ (X, d) \in P(\mathcal{E}) \times \{0, \ldots, D\} \}$ where $P(\mathcal{E})$ denotes the power set of $\mathcal{E}$. $d \in \{0,\ldots,D\}$ is the current depth in the tree.
...
\begin{align*}
   ...
   &= -\alpha - p_l * \Bigg(\frac{1}{|X_l|}\sum_{(\boldsymbol{x}_i, y_i)\in X_l}l(y_i, T_l(\boldsymbol{x}_i))  + \alpha C(T_l)\Bigg)\\
   &\ - p_r * \Bigg(\frac{1}{|X_r|}\sum_{(\boldsymbol{x}_i, y_i)\in X_r}l(y_i, T_r(\boldsymbol{x}_i))  + \alpha C(T_r)\Bigg)\\
   &= -\frac{1}{N}\sum_{(\boldsymbol{x}_i, y_i)\in X}l(y_i, T(\boldsymbol{x}_i)) - \alpha (1 + p_l C(T_l) + p_r C(T_r))\\
...
%To limit memory usage of non-terminal nodes, instead of storing all the samples in $(X,d)$, we only store $d$ and the binary vector of size $N$, $x_{bin} = {(1_{\{\boldsymbol{x}_i \in X\}})}_{i\in \{1,\ldots,N\}}$. 
%Even then,
