...
\section{Why do we want new decision tree induction algorithms?}
...
We already saw in the introduction of the manuscript that decision trees are well studied in supervised learning (cf. section~\ref{sec:dt}).
...
More importantly, we follow the methodology of Grinsztajn, Oyallon and Varoquaux~\cite{grinsztajn2022tree} to benchmark DPDT against both CART and optimal trees on hard datasets. Following the same methodology, we compare boosted DPDT \cite{FREUND1997119} to boosted CART and to some deep learning methods and show clear superiority of DPDT.

\section{Related work}

To learn decision trees, greedy approaches like CART (algorithm~\ref{alg:cart}) iteratively partition the training dataset by taking splits optimizing a local objective such as the Gini impurity or the entropy. 
...
\end{align*}
where $C: \mathcal{T}_D \rightarrow \mathbb{R}$ is a complexity penalty that helps prevent or reduce overfitting such as the number of nodes~\cite{breiman1984classification,quantbnb}, or the expected number of splits to label a data\cite{how-eff}. The complexity penalty is weighted by $\alpha \in [0, 1]$. %For regression problems where $\mathcal{Y} \subset \mathbb R$, we use the squared error $l(y_i, T(\boldsymbol{x}_i)) = {(y_i - T(\boldsymbol{x}_i))}^2$. 
\end{definition}
...
Please note that, while we focus on classification tasks, our framework extends naturally to regression tasks.
