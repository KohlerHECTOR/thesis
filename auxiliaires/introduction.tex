...
Such continuous states MDP can be formalized as lMDPs\cite{fmdp}:

\begin{definition}[lMarkov decision process]\label{def:fmdp} A lMDP is an MDP\ref{def:mdp} where the state space is a hyperrectangle: $S\subseteq \mathbb{R}^n$.
\end{definition}

From now on, all the MDPs we study in the this manuscript are lunless stated otherwise. 
...
Note that discrete states MDP can be encoded into lMDP by one-hot encoding individual states.
