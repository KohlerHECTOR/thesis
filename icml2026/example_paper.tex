...
\twocolumn[
  \icmltitle{Limits of reinforcement learning for non-parametric decision trees in\\ Markov decision processes}
...
\begin{abstract}
  Decision trees are considered to be interpretable machine learning models because humans can read trees predictions from the root to the leaves.
  Learning decision trees to solve supervised learning problems is well understood: there exist many optimal and heuristic algorithms.
  Comparatively, learning decision tree policies for Markov decision processes (MDPs) is an emerging research direction.
  In this paper we empirically study reinforcement learning as a means to directly learn decision tree policies for an MDP in opposition to most existing work that focuses on imitating neural network policies.
  In particular, we show that learning a decision tree policy for an MDP is like learning an arbitrary policy in a partially observable Markov decision problem (POMDP).
  This parallel between decision tree learning with reinforcement learning and POMDPs helps us understand why in practice it is often easier to obtain a non-interpretable expert policy--a neural network--and then imitate it with a tree rather than learning the decision tree from scratch.
\end{abstract}
