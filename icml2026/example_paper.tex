...
\begin{figure}
    ...
    \end{tikzpicture}}
    \caption{Grid world MDP, optimal actions, and a depth-1 decision tree policy.}\label{fig:mdp-dt}
\end{figure}

\subsection{Decision tree policies}
While other interpretable policy classes exist, one conjecture from \cite{glanois-survey} is that interpretable models are all hard to optimize or learn because they are non-differentiable in nature.
This is something that will be key in our study of decision tree policy that we introduce next and that we illustrate in figure~\ref{fig:mdp-dt}.

\begin{definition}[Decision tree policy]\label{def:dt}
    A decision tree policy is a rooted tree $\pi_{\mathcal{T}} = (\mathcal{N}, E)$.
    Each internal node $\nu \in \mathcal{N}$ is associated with a test that maps an MDP state features $s_{ij} \in S$ to a Boolean.
    ...
    For any input $\boldsymbol{s} \in S$, the tree defines a unique path from root to leaf, determining the prediction $\pi_{\mathcal{T}}(\boldsymbol{s}) = a_l$ where $l$ is the reached leaf.
...
\end{tikzpicture}}
\caption{By masking the current state to the agent, we force it to take information-gathering actions.}
\end{figure}
...
This essentially results in three distinct problems: (i) learning an optimal standard Markovian policy in an MDP, (ii) learning an optimal standard Markovian policy in an IBMDP, and (iii) learning an optimal deterministic partially observable policy in a POIBMDP.
...
Suppose an associated IBMDP (definition \ref{def:ibmdp}) with two IGAs: $\langle x, 1\rangle$ that tests if $x\leq 1$ and $\langle y, 1\rangle$ that tests if $y\leq 1$.
