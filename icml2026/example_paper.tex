...
Approaches to interpret machine learning models is either local or global~\cite{glanois-survey}.
Global interpretability methods constrain the model class so that the model's computations are transparent or verifiable by construction.
On the other hand, local interpretability methods--also called explainability methods or post-hoc methods~\cite{lipton}--provide interpretation of specific model computations. 
Such local interpretations can take various forms: linear models fitting some regions of the decision space~\cite{lime}, saliency maps on visual inputs~\cite{Puri2020Explain}, feature importance for the prediction such as SHAP~\cite{shap}, or attention-based highlighting~\cite{attention}.
The major drawback of local methods is that, to be interpreted, model predcitions are approximated and might not be faithful to the underlying black-box computations~\cite{Atrey2020Exploratory}.
For safety-critical settings, this motivates research on global approaches that return models interpretable by design such as decision trees~\cite{breiman1984classification}.


Global approaches are either direct or indirect~\cite{milani-survey}. 
Direct algorithms, such as decision tree learning~\cite{breiman1984classification}, \textit{directly} learn an interpretable model optimizing some objective, e.g. accuracy~\cite{oct}, or cumulative rewards.
One key challenge motivating this work is that decision tree learning is well-developed for classification and regression tasks~\cite{lookahead,binoct,murtree,blossom,pystreed} but not for sequential decision making.

Most existing research on interpretable sequential decision making has focused on developing indirect methods that first learn a non-interpretable model (e.g., neural network), and then fit an interpretable model (e.g., a decision tree) that emulates this black-box using imitation learning~\cite{viper,PIRL}.
Verifiable Reinforcement learning via Policy Extraction, or VIPER~\cite{viper}, is a strong indirect method to learn decision tree models for sequential decision making. VIPER first trains a neural network model with reinforcement learning~\cite{sutton} and then fits a decision tree to minimize the disagreement between the neural network and the tree outputs given the same inputs~\cite{dagger}.
They show that the obtained decision tree models, in addition to being transparent and to perform well on the underlying sequential decision making task, are also fast to verify in the formal sense of the term~\cite{maraboupy}.
Other indirect methods have imitated neural expert wtih models other than decision trees, e.g. Programmatically Interpretable Reinforcement Learning (PIRL)~\cite{PIRL} synthesizes programs in a domain-specific language. 
...
Such shortcuts can be, for example, following the shadow of someone instead of actually following someone because for the model they lead to the same reactions~\cite{scobots,shindo2024blendrl,nudge,ocatari}.
