\chapter{Appendix for part \ref{part3}}
\label{chap-app-imit}

In this chapter we provide insights on the utility of oblique decision tree policies to control certain MDPs (cf. definition~\ref{def:mdp}).
We also provide the full experimental results obtained with the methodology developed in part~\ref{part3}.
...
\begin{figure}[b]
    ...
    %\vspace{3mm}
    \caption{An oblique decision tree policy trained with PPO actions for different state space partitions of the Pong environment.}
    \label{fig:pong_states}
\end{figure}

One can imitate expert neural policies with programs that test linear combinations of features. Many experts learn oblique or more complex decision rules over an MDP state space.
This is illustrated in figure~\ref{fig:pong_states} where a PPO neural expert creates oblique partitions of the state-space for the Pong environments. Programs that test only individual features would fail to fit this partition (cf. figure~\ref{fig:pong_states}). 
...
We provide the interpretability-performance trade-offs of all the tested environments using the methodology from part~\ref{part3}.
\begin{figure}
    \centering
