...
\begin{figure}
    ...
    \scalebox{0.85}{\begin{tikzpicture}[
        ...
        \node<4->[decision] (tree4_root) at (11.5,2) {$x \leq 1$};
        \node<4->[leaf] (tree4_right) at (10.5,0) {$\rightarrow$};
        \node<4->[leaf] (tree4_left) at (12.5,0) {$\leftarrow$};
        \draw<4->[->] (tree4_root) -- (tree4_right) node[edge_label, above left] {True};
        \draw<4->[->] (tree4_root) -- (tree4_left) node[edge_label, above right] {False};
        \tikzstyle{grid}=[draw, thick, fill=gray!10]


        % Tree 4: if x <= 0.5 move right else move left
        \node<5->[decision] (tree5_root) at (5,2) {$x \leq 1$};
        \node<5->[leaf] (tree5_right) at (4,0) {$\rightarrow$};
        \node<5->[leaf] (tree5_left) at (6,0) {$\downarrow$};
        \draw<5->[->] (tree5_root) -- (tree5_right) node[edge_label, above left] {True};
        \draw<5->[->] (tree5_root) -- (tree5_left) node[edge_label, above right] {False};
        \tikzstyle{grid}=[draw, thick, fill=gray!10]

    \end{tikzpicture}}
    \caption*{\onslide<6->{An optimal depth-1 decision tree policy and a sub-optimal depth-1 decision tree policy.}}
    \end{figure}
\end{frame}
...
\begin{figure}
    ...
    \caption*{\onslide<2->{Imitation learning works well in practice to get interpretable policies {\small \textcolor{purple}{\parencite{milani-survey,viper,dagger}}} but no optimality guarantees.}}
\end{figure}
\end{frame}
...
\begin{frame}{Contributions}
    \begin{enumerate}
        \pause
        \item Why is learning optimal interpretable policies for sequential decision making difficult?
        \pause
...
\end{tikzpicture}}
\caption*{Trajectory in an IBMDP of the grid world MDP {\small \textcolor{purple}{\parencite{topin2021iterative}}}. Actions build a decision tree policy and rewards control the interpretability-performance trade-off.}
\end{figure}
\end{frame}
...
\begin{alertblock}{Cons}
    \begin{itemize}
        ...
        \item Finding the best \textbf{deterministic} and \textbf{partially observable} policy is NP-hard {\small \textcolor{purple}{\parencite{littman1}}}!
    \end{itemize}
\end{alertblock}
\end{frame}
