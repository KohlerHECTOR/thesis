...
\begin{itemize}
    \item<2-> {
    \only<2-3>{Interpretability $\simeq$ time to reproduce the computations.}
    \only<4->{Interpretability $\simeq$ \textbf{\textcolor{blue}{runtime in seconds?}}}
    }
    \item<3-> {
        \only<3-4>{Interpretability $\simeq$ effort to read through the entire policy.}
        \only<5->{Interpretability $\simeq$ \textbf{\textcolor{blue}{size in bytes?}}}
    }
\end{itemize}
\end{block}}

\onslide<6->{\begin{alertblock}{How to compare policy of different classes?}
\begin{itemize}
    \item<7-> Different hardwares (CPUs vs GPUs).
    \item<8-> Different implementations (matrix operations vs fully sequentially) {\small \textcolor{purple}{\parencite{insight}}}
\end{itemize}
\end{alertblock}}
\end{frame}
...
\end{figure}}
\onslide<3->{Less parameters means more interpretability {\small \textcolor{purple}{\parencite{study-0}}}.}
\end{frame}
