...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{A generic decision tree of depth $D=2$. Classification And Regression Trees (CART, 1984), Optimal Classification Trees (2015). What about sequential decision making?}
\end{figure}
\end{frame}
...
\begin{itemize}
    \item Given an MDP $\mathcal{M}=\langle S, A, R, T, T_0 \rangle$, the goal of reinforcement learning for sequential decision making is to find a model, also known as a policy, $\pi: S \rightarrow A$ that maximizes the expected discounted sum of rewards:
    ...
    \item Value iteration, Q-learning, Sarsa, Deep Q Networks, PPO, \cdot
\end{itemize}
\end{frame}
...
\begin{frame}{Direct approach to interpretable SDM}
    \begin{itemize}
        \item Policy class: non-parametric trees\cite{topin2021iterative}.
        \item Optimize a trade-off of policy perofrmances $J(\pi)$ and interpretability.
    \end{itemize}
...
\end{tikzpicture}}
\caption{Topin et. al. propose iterative bounding Markov decision processes (IBMDP). Trajectories in IBMDPs corresponding to decision tree policy building. Not all IBMDP policies are equivalent to decision tree policies}
\end{figure}
\end{frame}
...
\begin{alertblock}{Cons}
    \begin{itemize}
        ...
        \item Finding the best \textbf{deterministic} and \textbf{partially observable} policy is NP-hard \cite{littman}!!
    \end{itemize}
\end{alertblock}
\end{frame}

\begin{frame}{Re-formulation}
    \textit{Q: Can we use reinforcement learning to directly optimize trade-offs of performance and interpretability in SDM?} $\equiv$ \\
    \textit{Q: How does RL perform for optimizing \textbf{deterministic} and \textbf{partially observable} policies in IBMDPs?
}
\end{frame}

\begin{frame}{Experimental result: RL cannot retrieve optimal depth-1 trees for the grid world MDP}
\begin{figure}
    \centering
...
\begin{frame}{Experimental result: for similar problems, RL succeeds more often in retrieving optimal Markovian policies than partially observable policies}
\begin{figure}
    \centering
    \includegraphics[width=0.65\textwidth]{../images/images_part1/algorithm_performance_comparison_flattened.pdf}
    \caption{Success rates of different (asymmetric) RL algorithms over thousands of runs when applied to learning deterministic partially observable policies in an IBMDP for the grid world or learning deterministic Markovian policies in the same IBMDP.}
\end{figure}
\end{frame}
...
\begin{figure}
    ...
    \textbf{We show that in theory, deterministic partially observable policies for classification IBMDPs ($\equiv$ decision tree policies) are in fact Markovian.}    
\end{figure}
\end{frame}

% \begin{frame}{Experimental result: for SDM problems $\equiv$ classification problems, RL can consistently retrieve optimal depth-1 decision tree policies}
% \begin{figure}
%     \centering
...
\begin{frame}{Perspectives for direct RL of decision tree policies.}
    \begin{itemize}
        ...
        \item Still a lot of open questions:
        \begin{enumerate}
            \item What are the pros and cons of fixing the policy tree structure a priori (paramteric trees, \cite{sympol})?
            \item Can we specifically design algorithms that learn deterministic partially observable policies\cite{Lambrechts}?
        \end{enumerate}
    \end{itemize}
...
\begin{frame}{Perspectives}
    \begin{itemize}
        \item Great we did a new SOTA decision tree induction.
    \end{itemize}
