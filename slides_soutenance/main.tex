...
\begin{minipage}{0.46\textwidth}
    \begin{itemize}
        ...
        \item<3-> Lots of succesful RL algorithms {\small \textcolor{purple}{\parencite{ppo}}}.
...
\begin{frame}{Local vs. global interpretability {\small \parencite{glanois-survey}}.}
    \only<2>{\begin{figure}
    ...
    \only<4>{\caption*{\textbf{\warning Multiple definitions \textcolor{purple}{\small \parencite{lipton}}}.}}
\end{figure}
    }
\end{frame}
...
\begin{frame}{RL for memoryless policies in POMDPs}
    \onslide<1->{\begin{alertblock}{RL for memoryless policies}
        ...
        \item<4-> Value-based RL converges to sub-optimal solutions \textcolor{purple}{\small \parencite{sarsa-pomdp}}.
    \end{itemize}}
    \end{alertblock}
    \onslide<5->{\begin{block}{Asymmetric RL}
        \begin{itemize}
            \item<6-> Access to hidden states during training but not at execution \textcolor{purple}{\small \parencite{pinto}}.
            \item<7-> Value-based $\rightarrow$ learns $Q(o, a)$ with TD targets $U(s, a)$  \textcolor{purple}{\small \parencite{baisero-dqn}}.
            \item<8-> Actor-critic\footnote{Although those return stochastic policies, we can be greedy.} $\rightarrow$ policy gradient on $\pi(o, a)$ using a critic $V(s)$ \textcolor{purple}{\small \parencite{baisero-ppo}}.
            \item<9-> Supposed to work better for our problem \textcolor{purple}{\small \parencite{justif-asym}}.
        \end{itemize}
    \end{block}}
\end{frame}
...
\end{figure}
\only<2>{\textbf{\textcolor{teal}{Partial observations are sufficient statistics off the full states in classification IBMDPs.}}}
\end{frame}
