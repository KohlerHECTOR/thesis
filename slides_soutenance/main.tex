...
\begin{frame}{Sequential decision making and reinforcement learning}
\begin{figure}
    \centering
    \begin{tikzpicture}[
        node distance=2.5cm,
...
\only<1-5>{\textbf{\warning No concensus on what interpretability is.}
    \begin{itemize}
        \item<2->Different evaluations (e.g. with or without humans?) \textcolor{purple}{\small \parencite{rigourous}}.
        \item<3->Set of properties (e.g. transparency) \textcolor{purple}{\small \parencite{lipton}}.
        \item<4->Do we need an additional model to interpret the policy's decisions? \textcolor{purple}{\small \parencite{lipton,milani-survey,glanois-survey}} 
    \end{itemize}}
...
\only<6>{\begin{figure}
        ...
        \end{subfigure}
        \caption*{Local interpretation (a.k.a. explainability) of different decisions with saliency maps \textcolor{purple}{\small \parencite{saliency}}.}
    \end{figure}}
    \only<7->{
        \begin{figure}
    \centering
    ...
    \end{tikzpicture}
    \only<7>{\caption*{Interpretable policy classes do not require additional computations to interpret decisions\footnote{\tiny Google image result for \textit{interpretable machine learning models} \url{https://fr.mathworks.com/discovery/interpretability.html}}.}}
\end{figure}
    }
\end{frame}
...
\end{figure}
\onslide<2->{Most research focuses on imitation learning of interpretable policies {\small \textcolor{purple}{\parencite{viper,milani-survey}}}.}\\
\end{frame}
...
\end{figure}
\only<9->{
\onslide<9->{Given an MDP $\mathcal{M}$ $\langle S, A, R, T \rangle$}\onslide<10->{, an IBMDP for $\mathcal{M}$ is an MDP $$\langle S \times O, A \cup A_{info}, (R, \zeta), (T_{info}, T)\rangle$$
where:}
\begin{itemize}
    \item<11-> $O$: observations of some state features.
    \item<12-> $A_{info}$: actions that gather information about some state features.
    ...
    \item<15-> \textbf{\warning \textcolor{purple}{\cite{topin2021iterative}:} deterministic memoryless policies $\pi_{po}:O\rightarrow A\cup A_{info}$ in an IBDMP for $\mathcal{M}$ are decision trees for $\mathcal{M}$.}
\end{itemize}}
\end{frame}
...
\begin{frame}{RL for memoryless policies in POMDPs}
    \onslide<1->{\begin{alertblock}{RL for memoryless policies}
        ...
        \item<4-> No optimality guarantees for value-based RL \textcolor{purple}{\small \parencite{learning-pomdp}}.
    \end{itemize}}
    \end{alertblock}
    \onslide<5->{\begin{block}{Asymmetric RL {\small \parencite{pinto}}}
        \begin{itemize}
            ...
            \item<8-> \textcolor{purple}{\cite{topin2021iterative}} uses asymmetric RL (not knowing it) for their experiments.
        \end{itemize}
    \end{block}}
\end{frame}
...
\end{figure}
\only<2->{\textbf{\textcolor{teal}{Partial observations are sufficient statistics of histories in IBMDPs for classification tasks}}\onslide<3->{\textbf{$\rightarrow$ \textcolor{teal}{easy to solve \textcolor{purple}{\small \parencite{chap2}}.}}}}
\end{frame}
...
\begin{itemize}
    ...
    \pause
    \item \textbf{Dynamic Programming Decision Trees (DPDT)}:\\
    \pause
...
\begin{frame}{Practical implementaion of DPDT}
\begin{figure}
    \centering
...
\begin{frame}{CART generates more diverse splits than Top B for DPDT}
    \begin{figure}
        \centering
        \includegraphics[width=0.7\textwidth]{splits_tsne_4x4_all.pdf}
        \caption*{T-SNE projection of candidate splits on the \texttt{bank} dataset.}
    \end{figure}
\end{frame}
...
\onslide<4->{\warning \textbf{Without humans?}}
\onslide<1->{\begin{block}{The notion of \textit{simulatability} {\small \parencite{lipton}}}
\begin{itemize}
    \item<2-> {
    \only<2-4>{Interpretability $\simeq$ time to reproduce the computations.}
    \only<5->{Interpretability $\simeq$ \textbf{\textcolor{blue}{runtime in seconds?}}}
    }
    \item<3-> {
        \only<3-5>{Interpretability $\simeq$ effort to read through the entire policy.}
        \only<6->{Interpretability $\simeq$ \textbf{\textcolor{blue}{size in bytes?}}}
    }
\end{itemize}
\end{block}}

\onslide<7->{\begin{alertblock}{How to compare policy of different classes?}
\begin{itemize}
    \item<8-> Different hardwares (CPUs vs GPUs).
    \item<9-> Different implementations (matrix operations vs fully sequentially) {\small \textcolor{purple}{\parencite{insight}}}
\end{itemize}
\end{alertblock}}
\end{frame}
...
\only<2>{\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{tree_sizes_memory_ppo_ci_ablation.pdf}
...
\end{figure}}
\begin{itemize}
    \item<4->Less parameters means more interpretability {\small \textcolor{purple}{\parencite{study-0}}}.
    \item<5->Not always true that "trees are more interpretable than neural networks".
\end{itemize}
\end{frame}
...
\begin{frame}{General perspectives}
    \begin{itemize}
        ...
        \\
        \pause
        $\rightarrow$ \textbf{\textcolor{teal}{Created opportunities for new decision tree algos for classif/regression (DPDT).}}
        \pause
