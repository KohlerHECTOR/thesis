...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \node<4-> at (-0.5,1.7) {\small{\textcolor{blue}{\textbf{Direct ?}}}};
        
        % Direct method - RL process
        \node<5->[rl] (rl_direct) at (-2,0) {
        ...
        \draw<2->[arrow] (nn_indirect) -- (sl_indirect) node[label, midway] {\tiny Generates data};
        \draw<2->[arrow] (sl_indirect) -- (dt_indirect) node[label, midway] {\tiny Learns};
        
    \end{tikzpicture}}
\end{figure}
\onslide<3->{\textbf{\warning Policies obtained indirectly optimize a surrogate objective rather than an MDP cumulative rewards.}}
\end{frame}
...
\begin{frame}{Contributions}
    \begin{enumerate}
        \pause
        \item Can we directly optimize a trade-off of interpretability and performance in SDM with RL?
        \pause
...
\begin{frame}{Deterministic partially observable policies in IBMDPs}
    ...
    \onslide<4->{\begin{alertblock}{RL for partially observable policies}
        ...
        \item<7-> Value-based RL converges to sub-optimal solutions \textcolor{purple}{\small \cite{learning-pomdp,sarsa-pomdp}}.
    \end{itemize}}
    \end{alertblock}
...
\only<1>{\begin{figure}
    \centering
    \includegraphics[trim={3cm 15.5cm 20.4cm 1cm},clip,width=0.3\textwidth]{../images/images_part1/algorithm_performance_comparison_flattened.pdf}
\end{figure}}
\only<2->{\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{../images/images_part1/algorithm_performance_comparison_flattened.pdf}
\end{figure}}
\pause
\vspace{-0.4cm}
...
\begin{frame}{Perspectives for direct RL of decision tree policies.}
    ...
    \end{itemize}
    \pause
    \begin{exampleblock}{For classification MDPs, decision tree policies are fully Markovian in associated IBMDPs}
    \pause
...
\begin{frame}{CART generates more diverse splits than Top B}
    \only<1>{\begin{figure}
        \centering
...
% \end{frame}
\setbeamercovered{invisible}
\begin{frame}{Is policy unfolding really necessary?}
...
\begin{frame}{Conclusion: interpretable SDM is a difficult research topic}
    \begin{itemize}
        ...
        \item \textbf{Fundamental challenges:} \textcolor{red}{\textbf{no definition}}. 
        \\
        \pause
