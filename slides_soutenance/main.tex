...
%Title page
\bibliography{../auxiliares/bibliography}
\title %optional
[Interpretable RL]{Interpretable Sequential Decision Making}
...
\begin{itemize}
    \item \textcolor{green}{PhD student} \textit{Interpretability, Decision Trees, and Sequential Decision Making} at Inria in Lille (2022-Decembre 2025).
    \item \textcolor{orange}{Looking for a postdoc.}
    \item \textcolor{red}{Want to become prof./researcher.}
\end{itemize}
\end{frame}
...
\begin{frame}{Machine learning (ML) to learn policies for sequential decision making}
\begin{figure}
\hspace{-1.3cm}
        \begin{tikzpicture}[
            node distance=2.5cm,
    ...
    \end{figure}
\end{frame}
% \begin{frame}{Desiderata of autonomous programs for sensitive real-world applications}
% \begin{itemize}
%     \item Verifiability~\cite{kohler2025evaluating}
%     \item Interpretability~\cite{kohler2024interpretable,kohler2025evaluating}
%     \item Performance~\cite{KOHLER20227,10.1145/3711896.3736868}
%     \item Reproducibility~\cite{mathieu2024adastop}
%     \item Fairness $\dots$
% \end{itemize}
% \AtNextBibliography{\tiny}
% \printbibliography
% \end{frame}
\begin{frame}{Interpretable ML for sequential decision making}
\begin{figure}
\hspace{-1.3cm}
        \begin{tikzpicture}[
            node distance=2.5cm,
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{\textbf{Heuristic} interpretability-performance trade-offs of different policy classes (what-if neural network is very sparsed?). Interpretability is often presented in opposition to performances.}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Root decision node
        \node[decision] (root) at (0,0) {$t_1(x_i)$};
        
        % Second level nodes
        \node[decision] (left_decision) at (-3, -2.5) {$t_2(x_i)$};
...
\begin{frame}{Decision trees for MDPs}
\begin{figure}
    \centering
    \begin{tikzpicture}[
        node distance=2.5cm,
...
\begin{figure}
    ...
    \caption{The RL objective values of the optimal policies from figure~\ref{example:grid} and of the decision tree policies from figure~\ref{fig:trees-intro}.}\label{fig:objectives}
\end{figure}
\end{frame}
...
\begin{frame}{Interpretability, decision trees, and sequential decision making}
\begin{itemize}
    \item How to learn optimal interpretable policies for sequential decision making?
    \item \textbf{How to leverage sequential decision making to learn interpretable \textit{classifiers for supervised learning}?}
...
\begin{enumerate}
    \item Decision trees are \textbf{interpretable}.
    \item Tree-based models perform really well on \textbf{tabular} data, often \textbf{better than deep neural nets} (L. Grinsztajn et. al. 2022).
\end{enumerate}   
\end{frame}

\begin{frame}{Optimal decision tree induction is NP-hard}
\begin{itemize}
    \item Greedy algorithms (C4.5, CART, ID3, \dots) \textcolor{red}{sub-optimal accuracy}, but time complexity in \textcolor{green}{$O(2^D)$}.
    \item Optimal algorithms (MurTree, OCT, STreeD, Branches (Jesse Read's work ;)), \dots) \textcolor{green}{optimal accuracy}, but time complexity in \textcolor{red}{$O((2Np)^D)$}.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item S: data subsets.
    \item Ac: test or leaf nodes that can be added to the tree.
...
% \begin{frame}{Objective equivalence}
% \begin{block}{Objective Equivalence}\label{prop:equiv}
...
\begin{itemize}
    \item<1-> Greedy algorithms consider only one candidate action in each state which is the test that minimizes some impurity criterion $\rightarrow$ \textcolor{green}{MDP state space size is $O(2^D)$}.
    \item<1-> Optimal algorithms consider all possible actions in each state $\rightarrow$ \textcolor{red}{MDP state space size is $O((2Np)^D)$}.
    \item<2->  Let's choose candidate actions adaptively $\rightarrow$ for each MDP state consider $B$ actions: \textcolor{blue}{state space size is $O((2B)^D)$}.
\end{itemize}
\end{frame}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{../images/figures/schematic_cart_node_select.pdf}
\end{figure}
\end{frame}

\begin{frame}{Comparing trees accuracy versus complexity}
\begin{table}
    \tiny
...
\begin{frame}{DPDT trees generalization}
    \begin{figure}
     ...
     \begin{subfigure}[b]{0.48\textwidth}
         ...
         \caption{DPDT trees vs. other trees}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.48\textwidth}
         \centering
...
\end{figure}
\end{frame}
\begin{frame}{DPDT generalization}
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{../images/figures/tab_bench/random_search_classif_numerical_depth5.pdf}
\end{figure}
\end{frame}
\begin{frame}{Boosting DPDT}
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{../images/figures/tab_bench/random_search_classif_numerical_boosting_all_notgb.pdf}
\end{figure}
\end{frame}
...
\begin{itemize}
    \item The notion of \textit{simulatability}~\cite{lipton}:
    \begin{enumerate}
...
\begin{frame}{Empirical validation: obtaining $\sim40000$ policies from different classes}
    ...
    \caption{Imitation Learning to get policies from different classes (behaviour cloning, DAgger, VIPER)~\cite{viper,dagger,behavior-cloning}. The experts (a.k.a. oracles or teachers) neural network policies are obtained from the \texttt{stable-baselines3 zoo}~\cite{zoo}.}
    \label{fig:imit}
\end{figure}
\end{frame}
...
%   \toprule
%   Envs & Behav. Clon. & Behav. Clon. & Dagger & Dagger & VIPER & VIPER\\
...
\begin{frame}{Metrics}
\begin{enumerate}
    \item For each policy class and each environment, we keep only the best policy in terms of performance for the task.
    \item We measure the interpretability of each best-in-class policy on dedicated CPUs.
    \item To measure interpretability, we track two metrics:
    \begin{enumerate}
        \item Average inference time in seconds to predict actions given states.
        \item Space in memory in bytes.
    \end{enumerate}
\end{enumerate}
\end{frame}


\begin{frame}{Result 1: unfolding policies is necessary to respect consensus}
\begin{figure}
    \centering
...
\begin{frame}{Result 2: verification time does scale with step inference time}
    \begin{figure}[ht]
    \centering
...
\begin{frame}{Result 3: there is no dominating policy class for all environments}
\begin{figure}
    \centering
...
\begin{frame}{Take home messages}
    \begin{enumerate}
        \item Because there is no dominating class for all problems in terms of interpretability-performance trade-offs, popular beliefs such as "trees are more interpretable than neural networks" should be used with caution.
        \item This further motivates the use of the proposed methodology when comparing policies from different classes.
    \end{enumerate}
\end{frame}
...
\end{itemize}
All the policy programs are available on github \url{https://github.com/KohlerHECTOR/interpretable-rl-zoo}
\end{frame}
