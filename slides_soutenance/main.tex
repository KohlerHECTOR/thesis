...
\begin{frame}{Machine learning of policies for sequential decision making}
\begin{figure}
\hspace{-1.3cm}
        \begin{tikzpicture}[
            node distance=2.5cm,
...
\begin{frame}{Research direction}
    \textbf{\large How to learn interpretable policies for sequential decision making?}
\end{frame}
%% ------------- FORMALISM ------------------------------
\begin{frame}{Interpretable policies}
\begin{figure}
    \centering
...
\begin{frame}{Reinforcement learning objective}
    \begin{itemize}
    \item Given an MDP $\mathcal{M}=\langle S, A, R, T, T_0 \rangle$ (cf. definition~\ref{def:mdp}), the goal of reinforcement learning for sequential decision making is to find a model, also known as a policy, $\pi: S \rightarrow A$ that maximizes the expected discounted sum of rewards:
...
where $0< \gamma\leq 1$ is the discount factor that controls the trade-off between immediate and future rewards.
    \item Value iteration, Q-learning, Sarsa, Deep Q Networks, PPO, ...
    \end{itemize}
\end{frame}
...
% -------------- Existing works --------------
\begin{frame}{Machine learning of interpretable models for supervised learning}
    \begin{itemize}
...
where $C: \mathcal{F} \rightarrow \mathbb{R}$ is a penalty for regularization
        \item Classification And Regression Trees (CART, 1984), Optimal Classification Trees (2015)
        \item What about the RL objective for sequential decision making?
    \end{itemize}
\end{frame}

\begin{frame}{Two ways to get interpretable policies for sequential decision making}
\begin{figure}
    \centering
    \begin{tikzpicture}[
        node distance=2cm,
...
\begin{figure}
    ...
    \caption{Imitation learning to get interpretable policies (DAgger, VIPER)~\cite{viper,dagger}.}
\end{figure}
\end{frame}
...
\begin{itemize}
    ...
    \item How to leverage sequential decision making to learn interpretable \textit{classifiers for supervised learning}?
...
\end{tikzpicture}
\caption{An IBMDP trajectory when the base MDP is 2$\times$2 grid world.}
\end{figure}
\end{frame}
