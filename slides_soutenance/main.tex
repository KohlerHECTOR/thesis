...
\begin{figure}
    ...
    \end{tikzpicture}}
\end{figure}
\onslide<4->{\textbf{\warning Policies obtained indirectly optimize a surrogate objective rather than a trade-off between interpretability and cumulative rewards.}}
\end{frame}
...
\begin{frame}{Contributions}
    \begin{enumerate}
        ...
        \pause
        \item How to leverage DM to learn interpretable classifiers for supervised learning?
        \pause
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        % Add state labels clockwise from bottom left
        \node<2-> at (7.5,0.5) {{\color{teal} $\boldsymbol{\rightarrow}$}};
        \node<2-> at (8.5,0.5) {$\star$};
        \node<2-> at (8.5,0.7) {{\color{teal} $\boldsymbol{\uparrow}$}};
        \node<2-> at (8.5,0.3) {{\color{teal} $\boldsymbol{\downarrow}$}};
        \node<2-> at (8.7,0.5) {{\color{teal} $\boldsymbol{\rightarrow}$}};
        \node<2-> at (8.3,0.5) {{\color{teal} $\boldsymbol{\leftarrow}$}};
        \node<2-> at (8.5,1.5) {{\color{teal} $\boldsymbol{\downarrow}$}};
        \node<2-> at (7.2,1.3) {{\color{teal} $\boldsymbol{\downarrow}$}};
        \node<2-> at (7.5,1.5) {{\color{teal} $\boldsymbol{\rightarrow}$}};
    \end{tikzpicture}}
...
\begin{frame}{Direct RL of decision tree policies with iterative bounding Markov decision processes}
\pause
{\small 
Given an MDP $\mathcal{M}$ $\langle S, A, R, T \rangle$, an associated iterative bounding Markov decision process (IBMDP, {\small \cite{topin2021iterative}}) $\mathcal{M}_{IB}$ is an MDP:
$\langle \overbrace{S \times O}^{\text{Augmented state space}}, \underbrace{A \cup A_{info}}_{\text{Augmented action space}}, \overbrace{(R, \zeta)}^{\text{Augmented rewards}}, \underbrace{(T_{info}, T)}_{\text{Augmented transitions}}\rangle$}
\pause
\begin{itemize}
    \pause
    \item $O$: observations of some state features bounds.
    \pause
    \item $A_{info}$: actions that gather informations about some state features.
    \pause
    \item $\zeta$: reward for taking $a\in A_{info}$.
    \pause
    \item $T_{info}$: transitions following $a\in A_{info}$ that update the feature bounds with gathered info.
\end{itemize}
% \small
% \begin{itemize}
...
\begin{exampleblock}{IBMDPs promises}
    \begin{itemize}
        \pause
        \item No need to design new algorithm: we can use RL.
        \pause
...
\begin{frame}{Re-formulation}
    ...
    \pause
    \begin{alertblock}{Hard problem}
        Finding the best \textbf{deterministic} and \textbf{partially observable} policy is NP-hard \textcolor{purple}{\parencite{littman1}}!
    \end{alertblock}
\end{frame}
