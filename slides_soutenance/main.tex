...
\end{figure}
Algorithms are well understood for non-sequential learning: CART, OCT \cite{breiman1984classification,oct,murtree,binoct,quantbnb} \dots. What about SDM?
\end{frame}
...
\begin{itemize}
    \item The goal of RL (\cite{sutton}) for SDM is to find a policy, $\pi: S \rightarrow A$ that maximizes:
    \begin{align*}
...
\begin{figure}
    ...
    \caption{Imitation learning works well in practice to get interpretable policies (\cite{milani-survey,viper,dagger}) but no optimality guarantees.}
\end{figure}
\end{frame}
...
\begin{frame}{Perspectives for direct RL of decision tree policies.}
    \begin{itemize}
        \item Interpretability for SDM problems is difficult because for most problems there is \textcolor{red}{\textbf{partial observability}}.
        \item Should we focus on indirect approach? Hybrid approaches (\cite{parbhoo})?
        \item Fixing the policy tree structure a priori (paramteric trees, \cite{sympol})?
        \item Design algorithms that learn deterministic partially observable policies (\cite{lambrechts2025informed,justif-asym})?
    \end{itemize}
...
\begin{itemize}
    ...
    \end{align*}
    \item Tree-based models are \textbf{interpretable} and perform really well on \textbf{tabular} data, often \textbf{better than deep neural nets} (\cite{grinsztajn2022tree}).
\end{itemize}
\end{frame}

\begin{frame}{Optimal decision tree induction is NP-hard}
\begin{itemize}
    \item Greedy algorithms (\cite{breiman1984classification,ID3,c45}) \textcolor{red}{\textbf{sub-optimal accuracy}}, but time complexity in \textcolor{green}{$\boldsymbol{O(2^D)}$}.
    \item Optimal algorithms (\cite{oct,murtree,pystreed,chaouki2024branchesfastdynamicprogramming} \dots) \textcolor{green}{\textbf{optimal accuracy}}, but time complexity in \textcolor{red}{$\boldsymbol{O((2Np)^D)}$} (NP-hard \cite{npcomplete}).
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item Less parameters mean more interpretability (\cite{study-0,study-4,study-5,study-6,study-7}).
    \item Time to formally verify a policy decreases with interpretability (\cite{viper,lens-complexity}).
\end{itemize}
\end{exampleblock}
\end{frame}
