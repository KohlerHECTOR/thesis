...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption*{\textbf{Heuristic} interpretability-performance trade-offs of different policy classes. \warning No definition of interpretability in machine learning models!}
\end{figure}
\end{frame}
...
\end{figure}
\pause
Successful algorithms for non-sequential learning: {\small \textcolor{purple}{\parencite{breiman1984classification,oct,murtree,binoct,quantbnb}}} \dots What about SDM?
\end{frame}
...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        % Direct method box
        \node<4->[method_box] (direct_box) at (-0.5,0) {};
        \node<4-> at (-0.5,1.7) {\small{Direct}};
        
        % Direct method - RL process
        \node<1->[rl] (rl_direct) at (-2,0) {
        ...
        % Direct method arrow
        \draw<2->[arrow] (rl_direct) -- (dt_direct) node[label, midway] {\tiny Learns};
        
        % Indirect method box
        \node<4->[method_box_indirect] (indirect_box) at (7.1,0) {};
        \node<4-> at (6.8,1.7) {\small{Indirect}};
    ...
    \end{tikzpicture}}
\end{figure}
\onslide<5->{\warning Policies obtained indirectly optimize a surrogate objective rather than a trade-off between interpretability and cumulative rewards.}
\end{frame}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        % Add state labels clockwise from bottom left
        \node<2-> at (7.5,0.5) {{\color{green} $\rightarrow$}};
        \node<2-> at (8.5,0.5) {$\star$};
        \node<2-> at (8.5,0.7) {{\color{green} $\uparrow$}};
        \node<2-> at (8.5,0.3) {{\color{green} $\downarrow$}};
        \node<2-> at (8.7,0.5) {{\color{green} $\rightarrow$}};
        \node<2-> at (8.3,0.5) {{\color{green} $\leftarrow$}};
        \node<2-> at (8.5,1.5) {{\color{green} $\downarrow$}};
        \node<2-> at (7.2,1.3) {{\color{green} $\downarrow$}};
        \node<2-> at (7.5,1.5) {{\color{green} $\rightarrow$}};
    \end{tikzpicture}}
...
\begin{figure}
    \centering
    \includegraphics[trim={0 0 15.5cm 0},clip,width=0.6\textwidth]{../images/images_part1/base_mdp.pdf}
...
\begin{frame}{Contributions}
    \begin{enumerate}
        \pause
        \item How difficult is it to directly optimize a trade-off of interpretability and performance in SDM?
        \pause
        \item How to leverage sequential decision making to learn interpretable classifiers for supervised learning?
        \pause
        \item How to measure policy interpretability in sequential decision making?
    \end{enumerate}
\end{frame}
...
\begin{frame}{Direct RL of decision tree policies with iterative boundoing Markov decision processes}

Given an MDP $\mathcal{M}$ $\langle S, A, R, T \rangle$, an associated iterative bounding Markov decision process (IBMDP, {\small \cite{topin2021iterative}}) $\mathcal{M}_{IB}$ is a tuple:
$\langle \overbrace{S \times O}^{\text{Augmented state space}}, \underbrace{A \cup A_{info}}_{\text{Augmented action space}}, \overbrace{(R, \zeta)}^{\text{Augmented rewards}}, \underbrace{(T_{info}, T, T_0)}_{\text{Augmented transitions}}\rangle$

% \small
% \begin{itemize}
% \item<2-> $O\subsetneq S^2 =  [L_1, U_1]\times \dots \times [L_p, U_p] \times [L_1, U_1]\times \dots \times [L_p, U_p]$. 
% An IBMDP state is $\boldsymbol{s}_{IB} = (\overbrace{s_1, \dots, s_p}^{\boldsymbol{s}\in S}, \underbrace{L_1, U_1, \dots, L_p, U_p}_{\boldsymbol{o}\in O})$.
% \item<3-> $A_{info}$ are \textit{information gathering} actions: $\langle j, v \rangle$ where $j$ is a state feature index $1 \leq j \leq p$ and $v$ is a real number between $L_j$ and $U_j$. 
% \item<4-> $\zeta$ is a reward signal for taking an information gathering action.
% \item<5-> $T_{info}: S\times O \times( A_{info} \cup A )\rightarrow \Delta (S\times O)$ is the transition function of IBMDPs: 
...
% \end{itemize}
\pause
\begin{exampleblock}{IBMDPs promises}
    \begin{itemize}
        \pause
...
\begin{tikzpicture}[scale=0.6]
    ...
    \node<1-> at (2,8.5) {\small $\boldsymbol{s}_t=(0.5, 1.5)$};
    ...
    \node<6-> at (7,8.5) {\small $\boldsymbol{s}_t=(0.5, 1.5)$};
    ...
    \node<10-> at (12,8.5) {\small $\boldsymbol{s}_t=(1.5, 1.5)$};
    ...
    \node<14-> at (17,8.5) {\small $\boldsymbol{s}_t=(1.5, 1.5)$};
    ...
    \node<17-> at (22,8.5) {\small $\boldsymbol{s}_t=(1.5, 0.5)$};
...
\end{figure}
\begin{itemize}
    \item<19->{\small \textbf{Determinstic} and \textbf{partially observable} policies$\implies$ decision trees.}
    \item<20->{\small \warning Finding the best \textbf{deterministic} and \textbf{partially observable} policy is NP-hard \textcolor{purple}{\parencite{littman1}}!}
\end{itemize}
\end{frame}
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{../images/images_part1/algorithm_performance_comparison_flattened.pdf}
\end{figure}
Success rates when learning deterministic partially observable or Markovian policies in the same IBMDP.
\pause
...
\begin{figure}
    ...
    \end{tikzpicture}
\end{figure}
\onslide<4->{Classification MDP and the unique optimal depth-1 tree.}\\
\onslide<5->{\textbf{\warning Deterministic partially observable policies for classification IBMDPs ($\Leftrightarrow$ decision tree policies) are in fact Markovian.}}
\end{frame}

\begin{frame}{Result: RL can retrieve optimal depth-1 trees for the toy classification MDPs}
\begin{figure}
    \centering
...
\begin{itemize}
    \pause
    \item $N$ data points. Each $x_i$ is described by $p$ features and has a label $y_i \in {\mathcal Y}$.
    \begin{align*}
    ...
    \pause
    \item Greedy algorithms \textcolor{red}{\textbf{sub-optimal accuracy}}, but \textcolor{green}{$\boldsymbol{O(2^D)}$} operations {\small \textcolor{purple}{\parencite{breiman1984classification,ID3,c45}}} .
    \pause
    \item Optimal algorithms, \textcolor{green}{\textbf{optimal accuracy}}, but \textcolor{red}{$\boldsymbol{O((2Np)^D)}$} operations (NP-hard) {\small \textcolor{purple}{\parencite{oct,murtree,pystreed,chaouki2024branchesfastdynamicprogramming,npcomplete}}}.
    \pause
...
\begin{block}{Intuition}
    The induction of a decision tree is made of a sequence of decisions: at each node, we must decide whether it is better to split (a subset of) $\mathcal{E}$, or %assign a class label and 
...
\begin{itemize}
    ...
    \pause
    $\rightarrow$ \textcolor{green}{\textbf{MDP state space size is} $\boldsymbol{O(2^D)}$}.
    \pause
    ...
    \pause
    \item Let's choose candidate actions adaptively \\
    \pause
...
\begin{figure}
    ...
    \pause
    \caption*{DPDT was presented at the 31st ACM SIGKDD conference.}
\end{figure}
\end{frame}

\begin{frame}{Theory of generating candidate splits with CART}
    \pause
...
\begin{frame}{DPDT trees can be strictly better than greedy trees}
    \begin{figure}
        \centering
        \includegraphics[width=1\textwidth]{Présentation sans titre (19).png}
    \end{figure}
\end{frame}
\begin{frame}{DPDT trees can be strictly better than greedy trees}
    \begin{figure}
        \centering
        \includegraphics[width=1\textwidth]{Présentation sans titre (20).png}
    \end{figure}
\end{frame}
% \begin{frame}{DPDT trees can be strictly better than greedy trees}
%     \begin{figure}
...
\begin{table}
    ...
    \tiny
    \scalebox{0.93}{\begin{tabular}{l|cc||cc|cc|c||cc|cc}
    \toprule
...
\pause
\begin{alertblock}{Challenges {\small \parencite{glanois-survey,lipton,rigourous}}}
\begin{itemize}
    \pause
    \item No definition of interpretability.
    \pause
    \item Measuring might require humans.
\end{itemize}
\end{alertblock}
\pause
\begin{exampleblock}{The notion of \textit{simulatability} {\small \parencite{lipton}}}
\begin{itemize}
    \pause
    \item Interpretability $\simeq$ how long for human to make the same computations.
    \pause
    \item Interpretability $\simeq$ how much effort for a human to read through the entire policy.
    \pause
    \item Less parameters mean more interpretability {\small \textcolor{purple}{\parencite{study-0,study-4}}}.
    \pause
    \item Time to formally verify a policy decreases with interpretability {\small \textcolor{purple}{\parencite{lens-complexity}}}.
\end{itemize}
\end{exampleblock}
\end{frame}
...
\begin{frame}{Conclusion: interpretable SDM is a difficult research topic}
    \begin{itemize}
        ...
        \pause
        \item \textcolor{green}{\textbf{Decision trees offer good inductive bias for SDM in games or tabular data}}.
    \end{itemize}
    \pause
    \begin{block}{My hope}
