...
\only<1-4>{\textbf{\warning No concensus on what interpretability is.}
    \begin{itemize}
        ...
        \item<4->How to interpret the policy's decisions? \textcolor{purple}{\small \parencite{lipton,milani-survey,glanois-survey}} 
    \end{itemize}}
...
\only<6>{\begin{figure}
        ...
        \end{subfigure}
        \caption*{Local interpretation (a.k.a. explainability) requires additional computations \textcolor{purple}{\small \parencite{saliency}}.}
    \end{figure}}
    \only<7->{
        \begin{figure}
    \centering
    ...
    \end{tikzpicture}
    \only<7>{\caption*{Interpretable policy classes do not require additional computations\footnote{\tiny Google image result for \textit{interpretable machine learning models} \url{https://fr.mathworks.com/discovery/interpretability.html}}.}}
\end{figure}
    }
\end{frame}
...
\end{figure}
\onslide<1->{Most research focuses on imitation learning of interpretable policies {\small \textcolor{purple}{\parencite{viper,milani-survey}}}.}\\
\end{frame}
...
\begin{frame}{Fast like greedy trees, accurate like optimal trees}
    ...
    \begin{itemize}
        \item<2-> DPDT trees are not worse than greedy trees.
...
\end{figure}}
\begin{itemize}
    \item<3->\warning Less parameters means more interpretability {\small \textcolor{purple}{\parencite{study-0}}}.
