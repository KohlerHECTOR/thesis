...
\begin{frame}{Deterministic partially observable policies in IBMDPs}
    ...
    \begin{block}{Partially observable}
        Otherwise
    \end{block}
    \begin{alertblock}{RL for POMDPs}
        Finding the best \textbf{deterministic} and \textbf{partially observable} policy is NP-hard \textcolor{purple}{\parencite{littman1}}!
        
    \end{alertblock}
\end{frame}

\begin{frame}{Asymmetric RL for POMDPs}
    \begin{exampleblock}{Access the hidden state during training}
        \begin{itemize}
            \item Value based $\rightarrow$
            \item Actor-critic $\rightarrow$\footnote{Although those return stochastic policies, we can be greedy.}
        \end{itemize}
    \end{exampleblock}
\end{frame}
