...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \draw<3->[arrow] (environment) to[bend left=30] node[left] {
            \begin{minipage}{2cm}
                \centering
                \includesvg[width=0.4cm]{../images/images_intro/patient-clipboard-svgrepo-com.svg}\\
                \small{Updated health status}
        ...
        %     \end{minipage}
        % } (nn_);
        \draw<3->[arrow_decision] (robot_) to node[above] {\small{Recommends}} (doctor_);
...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \node<1-3>[robot] (robot_) at (-2.1,3.4) {
            \begin{minipage}{1.5cm}
                \centering
                \includesvg[width=0.4cm]{../images/images_intro/robot-svgrepo-com.svg}\\
                \small{Policy}
        ...
        \draw<1-3>[arrow] (environment) to[bend left=30] node[left] {
            \begin{minipage}{2cm}
                \centering
                \includesvg[width=0.4cm]{../images/images_intro/patient-clipboard-svgrepo-com.svg}\\
                \small{Updated health status}
        ...
        \draw<4->[arrow] (environment) to[bend left=30] node[left] {
            \begin{minipage}{2cm}
                \centering
                \includesvg[width=0.4cm]{../images/images_intro/patient-clipboard-svgrepo-com.svg}\\
                \small{Updated health status}
        ...
        \draw<1-3>[arrow_decision] (robot_) to node[above] {\small{Recommends}} (doctor_);
        \draw<4->[arrow_decision] (nn_) to node[above] {\small{Recommends}} (doctor_);
        % \draw<1-3>[arrow_decision] (doctor_) to node[below] {\small{Interprets}} (robot_);
        \draw<5->[arrow_decision, red] (doctor_) to node[below] {\small{Cannot interpret}} (nn_);
        ...
        % ML learning arrows
        \draw<3->[arrow] (environment) to[bend left=40] node[left] {
            \begin{minipage}{2cm}
                \centering
                \includesvg[width=0.5cm]{../images/images_intro/patient-clipboard-svgrepo-com.svg}\\
                \small{Treatment outcomes}
        ...
        \draw<3>[arrow] (ml) to[bend left=20] node[above] {
            \begin{minipage}{1.5cm}
                \centering
                \small{Updates}
            \end{minipage}
        } (robot_);
        \draw<4->[arrow] (ml) to[bend left=20] node[above] {
            \begin{minipage}{1.5cm}
                \centering
                \small{Updates}
            \end{minipage}
...
\begin{frame}{Interpretability?}
    \only<2>{Local vs. global interpretability \textcolor{purple}{\small \cite{milani-survey,glanois-survey}}.}
    \only<3>{\begin{figure}
        ...
        \end{subfigure}
        \caption*{Saliency maps \textcolor{purple}{\small \cite{saliency}}.}
    \end{figure}}
    \only<4>{
        \begin{figure}
            \centering
            \includegraphics[width=0.9\textwidth]{../images/lime.png}
            \caption*{Local interpretable model-agnostic explanations (LIME) \textcolor{purple}{\small \cite{lime}}.}
        \end{figure}
    }
    \only<5->{
        \begin{figure}
    ...
    \end{tikzpicture}
    \only<6>{\caption*{Global interpretability $\rightarrow$ works for all states.}}
    \only<7>{\caption*{\textbf{\warning Multiple definitions \textcolor{purple}{\small \cite{lipton,rigourous,lens-complexity}}}.}}
\end{figure}
    }
\end{frame}
...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        % Direct method arrow
        \draw<5->[arrow] (rl_direct) -- (dt_direct) node[label, midway] {\tiny Learns};
...
\begin{frame}{RL for memoryless policies in POMDPs}
    \onslide<1->{\begin{alertblock}{RL for memoryless policies}
        \begin{itemize}
        \item<2-> Finding the best \textbf{deterministic} and memoryless policy in a POMDP is NP-hard \textcolor{purple}{\small \parencite{littman1}}!
        \item<3-> The best memoryless policy can be stochastic \textcolor{purple}{\small \cite{learning-pomdp}}.
        \item<4-> Value-based RL converges to sub-optimal solutions \textcolor{purple}{\small \cite{learning-pomdp,sarsa-pomdp}}.
    \end{itemize}}
    \end{alertblock}
    \onslide<5->{\begin{block}{Asymmetric RL}
        \begin{itemize}
            \item<6-> Access to hidden states during training but not at execution \textcolor{purple}{\small \cite{pinto}}.
            \item<7-> Value-based $\rightarrow$ learns $Q(o, a)$ with TD targets $Q(s, a)$  \textcolor{purple}{\small \cite{baisero-dqn}}.
            \item<8-> Actor-critic\footnote{Although those return stochastic policies, we can be greedy.} $\rightarrow$ policy gradient on $\pi(o, a)$ using a critic $V(s)$ \textcolor{purple}{\small \cite{baisero-ppo}}.
            \item<9-> Supposed to work better for our problem \textcolor{purple}{\small \cite{justif-asym}}.
        \end{itemize}
    \end{block}}
\end{frame}
...
\begin{frame}{Perspectives for direct RL of decision tree policies.}
    \begin{itemize}
        ...
        \pause
        \item Fixing the policy tree structure a priori (parametric trees, {\small \textcolor{purple}{\cite{sympol}}})?
        % \pause
...
\begin{itemize}
    ...
    \pause
    \item R: interpretability term $-\alpha$ and accuracies.
    \pause
...
Then $J_\alpha(\pi) = -{\mathcal L}_\alpha(E(\pi, s_0))$ where $E$ is an algorithm that extracts a decision tree from $\pi$ \textcolor{purple}{\small \cite{topin2021iterative}}.
\end{proposition}
\end{frame}
\setbeamercovered{invisible}
...
\setbeamercovered{invisible}
\begin{frame}{Large scale evaluation of DPDT trees generalization {\small \cite{grinsztajn2022tree}}}
    \begin{figure}
     \centering
...
\begin{frame}[noframenumbering]{Result: RL cannot retrieve optimal depth-1 trees for the grid world MDP}
\begin{figure}
    \centering
...
\begin{frame}[noframenumbering]{Result: decision tree policies for classification MDPs are standard Markovian policies in IBMDPs}
\begin{figure}
    \centering
    \begin{tikzpicture}[
        decision/.style={circle, draw, thick, fill=blue!20, text width=2.5em, text centered, minimum height=2.5em, font=\small},
...
\pause
\\
\textbf{\textcolor{teal}{Deterministic memoryless policies for classification IBMDPs ($\Leftrightarrow$ decision tree policies) are in fact Markovian.}}
\end{frame}

\begin{frame}[noframenumbering]{Result: RL can retrieve optimal depth-1 trees for the toy classification MDP}
\begin{figure}
    \centering
    \includegraphics[width=1\textwidth]{../images/images_part1/tree_distributions_classif.pdf}
