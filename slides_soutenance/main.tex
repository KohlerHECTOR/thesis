...
\begin{minipage}{0.4\textwidth}
    \begin{itemize}
        ...
        \item<4-> No interpretability concerns.
    \end{itemize}
\end{minipage}
\end{frame}
...
\end{figure}
\begin{itemize}
\item<2->{Most research focused on indirect learning of interpretable policies {\small \textcolor{purple}{\parencite{dagger,viper,PIRL,milani-survey,}}}.}\\
\item<3->{\warning Imitated policies optimize a surrogate objective rather than a trade-off between interpretability and cumulative rewards.}
\end{itemize}
\end{frame}
...
\end{figure}
\begin{itemize}
    \item<2->{\small \textbf{Determinstic} and \textbf{partially observable} policies$\leftrightarrow$ decision trees.}
    \item<3->{\small \warning Finding the best \textbf{deterministic} and \textbf{partially observable} policy is NP-hard \textcolor{purple}{\parencite{littman1}}!}
\end{itemize}
\end{frame}
...
\begin{frame}{Re-formulation}
    \textit{Q: Can we use reinforcement learning to directly optimize trade-offs of performance and interpretability in SDM?} \\ 
    \vspace{0.5cm}
    \hspace{5cm}{\huge $\Leftrightarrow$} \\
    \vspace{0.5cm}
...
\begin{figure}
    ...
    \caption*{Distributions of final tree policies learned with various (asymmetric) RL algorithms {\small \textcolor{purple}{\parencite{sutton,learning-pomdp,sarsa-pomdp,baisero-ppo,baisero-dqn}}} across 100 seeds.
...
\begin{figure}
    ...
    \textbf{We show that deterministic partially observable policies for classification IBMDPs ($\Leftrightarrow$ decision tree policies) are in fact Markovian.}
\end{figure}
\end{frame}
...
\begin{figure}
    ...
    \caption*{Distributions of final tree policies learned with various RL algorithms across 100 seeds.
    For each different performance-interpretability trade-off value $\zeta$, each point represent the share of different trees.
    }
\end{figure}
\end{frame}
...
\begin{frame}{Perspectives for direct RL of decision tree policies.}
    ...
    \textit{Q: Can we leverage SDM design new decision tree induction algorithms for the supervised learning setting?}
    \textbf{A: Yes!}
    \end{exampleblock}
\end{frame}
...
\begin{itemize}
    \item Greedy algorithms consider only one candidate action in each state which is the test that minimizes some impurity criterion $\rightarrow$ \textcolor{green}{\textbf{MDP state space size is} $\boldsymbol{O(2^D)}$}.
    \item Optimal algorithms consider all possible actions in each state $\rightarrow$ \textcolor{red}{\textbf{MDP state space size is} $\boldsymbol{O((2Np)^D)}$}.
    \item Let's choose candidate actions adaptively $\rightarrow$ for each MDP state consider $B$ actions: \textcolor{blue}{\textbf{state space size is} $\boldsymbol{O((2B)^D)}$}.
\end{itemize}
\end{frame}
...
\begin{frame}{Theoretical motivation}
    \begin{theorem}[DPDT trees are not worse than greedy trees] The greedy tree is always a solution of the MDPs we solve. Because we solve the MDPs exactly with DP, if the greedy tree is the best solution, DPDT will find it.
    \end{theorem}
    \begin{theorem}[DPDT trees can be strictly better than greedy trees] There exist a depth budget $D$ and a dataset for which DPDT trees are strictly better than greedy trees.
    \end{theorem}
\end{frame}
%-------------LONNNNNNNNNNNNNNG
...
\begin{frame}{Conclusion: interpretable machine learning is a difficult research topic}
    \begin{itemize}
        \item Technical challenges: \textcolor{red}{\textbf{partial observability in SDM, NP-hardness}}.\\ $\rightarrow$ Focus on indirect approaches and/or on POMDP research first.
        \item Fundamental challenges: \textcolor{red}{\textbf{no definition}}. \\ $\rightarrow$ Discuss with the community (InterpPol workshop).
