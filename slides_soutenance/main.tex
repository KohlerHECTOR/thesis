...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        % Decision Making Box
        \node<1-2>[decision_box] (decision_box) at (0,3.8) {};
        \node<3->[big_decision_box] (big_decision_box) at (0,3.8) {};
        ...
        \node<3-7>[robot] (robot_) at (-2.1,3.4) {
            \begin{minipage}{1.5cm}
                \centering
                \includesvg[width=0.4cm]{../images/images_intro/robot-svgrepo-com.svg}\\
                \small{Policy}
        ...
        \node<3->[agent] (doctor_) at (2.1,3.4) {
            \begin{minipage}{1.5cm}
                \centering
                \includesvg[width=0.4cm]{../images/images_intro/doctor-with-stethoscope-svgrepo-com.svg}\\
                \small{Doctor}
        ...
        % Arrows
        \draw<1-2>[arrow] (environment) to[bend left=30] node[left] {
            \begin{minipage}{2cm}
                \centering
        ...
        \draw<3-7>[arrow] (environment) to[bend left=30] node[left] {
            \begin{minipage}{2cm}
                \centering
                \includesvg[width=0.4cm]{../images/images_intro/patient-clipboard-svgrepo-com.svg}\\
                \small{Updated health status}
        ...
        \draw<3-7>[arrow_decision] (robot_) to node[above] {\small{Recommends}} (doctor_);
        ...
        \draw<3->[arrow] (doctor_) to[bend left=30] node[right] {
            \begin{minipage}{3cm}
                \centering
                \includesvg[width=0.5cm]{../images/images_intro/syringe-svgrepo-com.svg}\\
                \small{Administers chemotherapy}
...
\begin{minipage}{0.4\textwidth}
    \begin{itemize}
        \item<2-> RL {\small \textcolor{purple}{\parencite{sutton}}} aims to find a policy, $\pi: S \rightarrow A$ that maximizes:
        \begin{align*}
            \underset{\boldsymbol{s}_{t}\sim T}{\mathbb{E}}\left[\sum_{t=0}^{\infty} \gamma^t R(s_t, \pi(s_t))\right]
        \end{align*}

        \item<3-> Lots of succesful RL algorithms {\small \textcolor{purple}{\parencite{sutton,dqn,ppo}}}.
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption*{\textbf{Heuristic} interpretability-performance trade-offs of different policy classes.}
\end{figure}
\pause\textbf{\warning No definition of interpretability for machine learning models!}
\end{frame}
...
\end{figure}
\pause
Successful algorithms for classification/regression: {\small \textcolor{purple}{\parencite{breiman1984classification,oct,murtree,binoct,quantbnb}}} \dots
\\
\pause
...
\end{figure}
\onslide<2->{Most research focused on indirect learning of interpretable policies {\small \textcolor{purple}{\parencite{dagger,viper,PIRL,milani-survey,}}}.}\\
\end{frame}
...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        % Direct method box
        \node<5->[method_box] (direct_box) at (-0.5,0) {};
        \node<5-> at (-0.5,1.7) {\small{\textcolor{blue}{\textbf{Direct ?}}}};
        
        % Direct method - RL process
        \node<6->[rl] (rl_direct) at (-2,0) {
        ...
        % Direct method arrow
        \draw<6->[arrow] (rl_direct) -- (dt_direct) node[label, midway] {\tiny Learns};
        ...
        \draw<2->[arrow] (rl_indirect) -- (nn_indirect) node[label, midway] {\tiny Learns};
        \draw<3->[arrow] (nn_indirect) -- (sl_indirect) node[label, midway] {\tiny Generates data};
        \draw<3->[arrow] (sl_indirect) -- (dt_indirect) node[label, midway] {\tiny Learns};
        
    \end{tikzpicture}}
\end{figure}
\onslide<4->{\textbf{\warning Policies obtained indirectly optimize a surrogate objective rather than an MDP cumulative rewards.}}
\end{frame}
...
\begin{frame}{Contributions}
    \begin{enumerate}
        \pause
        \item Can we directly optimize a trade-off of interpretability and performance in SDM?
        \pause
        \item Can we leverage SDM to learn interpretable classifiers for supervised learning?
        \pause
...
\begin{frame}{Iterative bounding Markov decision processes~\cite{topin2021iterative}}
\begin{figure}
\centering
\only<1>{\scalebox{0.7}{
\begin{tikzpicture}[scale=0.6]
    ...
    \draw[thick, ->] (2,3) to[bend right=30] node[midway, below] {}(12,3);
    ...
    % \node[rectangle, draw, thick, fill=green!25, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_right) at (3,-3) {$\rightarrow$};
    % \node[rectangle, draw, thick, fill=green!25, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_left) at (6,-3) {$\downarrow$};
    ...
    % \node[rectangle, draw, thick, fill=green!5, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_right_new) at (3,-3) {$\rightarrow$};
    % \draw[->] (tree4_root) -- (tree4_right_new) node[font=\footnotesize, midway, above left] {True};
    % \node[rectangle, draw, thick, fill=green!25, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_right_new_new) at (3,-3) {$\rightarrow$};
    % \draw[->] (tree4_root) -- (tree4_right_new_new) node[font=\footnotesize, midway, above left] {True};

    \node at (12,7.5) {\small $t=1$};
    ...
    \draw[thick, ->] (12,3) to[bend right=30] node[midway, below] {} (22,3);
...
\begin{tikzpicture}[scale=0.6]
    ...
    % Curved arrow from t=0 to t=1
    \draw[thick, ->] (2,3) to[bend right=30] node[midway, below] {} (7,3);

    % % t=1: After AIG x≤0.5
    \node at (7,7.5) {\small $t=1$};
    ...
    % Curved arrow from t=1 to t=2
    \draw[thick, ->] (7,3) to[bend right=30] node[midway, below] {}(12,3);
    ...
    % Curved arrow from t=2 to t=4
    \draw[thick, ->] (12,3) to[bend right=30] node[midway, below] {} (17,3);
    ...
    \draw[thick, ->] (17,3) to[bend right=30] node[midway, below] {} (22,3);
    ...
    \node<5->[circle, draw, thick, fill=blue!5, text width=2em, text centered, minimum height=1.5em, font=\small] (tree4_root) at (9.5,0) {$x \leq 1$};
    \node<5->[rectangle, draw, thick, fill=green!25, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_right) at (8,-3) {$\rightarrow$};
    \node<5->[rectangle, draw, thick, fill=green!5, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_left) at (11,-3) {$\downarrow$};
    ...
    \node<4->[rectangle, draw, thick, fill=green!5, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_right) at (3,-3) {$\rightarrow$};
    \node<4->[rectangle, draw, thick, fill=green!5, text width=1.5em, text centered, rounded corners, minimum height=1em, font=\small] (tree4_left) at (6,-3) {$\downarrow$};
    \draw<4->[->] (tree4_root) -- (tree4_right) node[font=\footnotesize, midway, above left] {True};
    \draw<4->[->] (tree4_root) -- (tree4_left) node[font=\footnotesize, midway, above right] {False};

    
\end{tikzpicture}}
}
\end{figure}
\only<8>{\textbf{\warning To learn decision tree policies for an MDP we need to hide and allow agents to query information about state features.}}
\end{frame}
...
% \end{frame}
\setbeamercovered{transparent}
\begin{frame}{Deterministic partially observable policies in IBMDPs}
    \only<1-3>{\begin{exampleblock}{IBMDPs promises}
        \begin{itemize}
            \item<2-> No need to design new algorithm: we can use RL.
            \item<3-> IBMDP rewards trade-off naturally interpretability and performances.
        \end{itemize}
        \end{exampleblock}}
    \onslide<4->{\begin{alertblock}{RL for partially observable policies}
        \begin{itemize}
        \item<5-> Finding the best \textbf{deterministic} and \textbf{partially observable} policy is NP-hard \textcolor{purple}{\small \parencite{littman1}}!
        \item<6-> The best partially observable policy can be stochastic \textcolor{purple}{\small \cite{learning-pomdp}}.
        \item<7-> Value-based RL converges to sub-optimal solutions \textcolor{purple}{\small \cite{learning-pomdp}}.
    \end{itemize}}
    \end{alertblock}
    \onslide<8->{\begin{block}{Asymmetric RL}
        \begin{itemize}
            \item<9-> Access to hidden states during training but not at execution \textcolor{purple}{\small \cite{pinto}}.
            \item<10-> Value-based $\rightarrow$ learns $Q(o, a)$ with TD targets $Q(s, a)$  \textcolor{purple}{\small \cite{baisero-dqn}}.
            \item<11-> Actor-critic\footnote{Although those return stochastic policies, we can be greedy.} $\rightarrow$ policy gradient on $\pi(o, a)$ using a critic $V(s)$ \textcolor{purple}{\small \cite{baisero-ppo}}.
            \item<12-> Supposed to work better for our problem \textcolor{purple}{\small \cite{justif-asym}}.
        \end{itemize}
    \end{block}}
\end{frame}
...
% \end{figure}
% \pause
% Distributions of tree policies learned with (asymmetric) RL algorithms {\small \textcolor{purple}{\parencite{sutton,learning-pomdp,sarsa-pomdp,baisero-ppo,baisero-dqn}}} as a function of the interpretability reward $\zeta$.
% \pause
% \\
% \textcolor{blue}{\textbf{Are the poor performances due to partial observability?}}
% \end{frame}

\begin{frame}{Result: for similar problems, RL struggles more when there is partial observability}
\begin{figure}
    \centering
    \includegraphics[width=0.65\textwidth]{../images/images_part1/algorithm_performance_comparison_flattened.pdf}
\end{figure}
Success rates over thousands of RL runs with varying hyperparameters when learning different policies in the same IBMDP\footnote{\tiny We also observed similar results on classic controls and variants of the grid world MDP.}.
\pause
\textcolor{blue}{\textbf{Is it all for nothing?}}
\end{frame}

% \begin{frame}{Result: decision tree policies for classification MDPs are standard Markovian policies in IBMDPs}
% \begin{figure}
%     \centering
%     \begin{tikzpicture}[
%         decision/.style={circle, draw, thick, fill=blue!20, text width=2.5em, text centered, minimum height=2.5em, font=\small},
...
%     ]
%         % Tree 4: if x <= 0.5 move right else move left
%         \node<3->[decision] (tree4_root) at (8,2) {$x \leq 1$};
%         \node<3->[rectangle, draw, thick, fill=green!40, text width=2em, text centered, rounded corners, minimum height=2em, font=\small] (tree4_right) at (7,0) {};
%         \node<3->[rectangle, draw, thick, fill=red!40, text width=2em, text centered, rounded corners, minimum height=2em, font=\small] (tree4_left) at (9,0);
%         \draw<3->[->] (tree4_root) -- (tree4_right) node[edge_label, above left] {True};
%         \draw<3->[->] (tree4_root) -- (tree4_left) node[edge_label, above right] {False};
%         \tikzstyle{grid}=[draw, thick, fill=gray!10]
        
%         % Draw grid
%         \draw<2->[fill=green!40] (0, 0) rectangle (1,2);
%         \draw<2->[fill=red!40] (1, 0) rectangle (2,2);
...
% \pause
% \\
% \textbf{\textcolor{teal}{Deterministic partially observable policies for classification IBMDPs ($\Leftrightarrow$ decision tree policies) are in fact Markovian.}}
% \end{frame}
...
\begin{frame}{Perspectives for direct RL of decision tree policies.}
    \begin{itemize}
        \pause
        \item It seems that interpretability for SDM problems can be difficult to achieve because of \textcolor{red}{\textbf{partial observability}}.
        \pause
        \item Should we focus on indirect approaches? Hybrid approaches {\small \textcolor{purple}{\parencite{parbhoo}}}?
        \pause
        \item Fixing the policy tree structure a priori (paramteric trees, {\small \textcolor{purple}{\cite{sympol}}})?
        \pause
        \item Can other policies (programs, oblique trees, algebraic expressions\dots) be directly optimized with RL in IBMDPs?
        \pause
        \item Design algorithms that learn deterministic partially observable policies {\small \textcolor{purple}{\parencite{lambrechts2025informed,justif-asym}}}?
    \end{itemize}
    \pause
    \begin{exampleblock}{We show that decision tree policies for supervised learning tasks are fully observable IBMDP policies.}
    \pause
    \textit{Q: Can we leverage SDM to design new decision tree induction algorithms for the supervised learning setting?}
    \pause
    \textbf{A: Yes!}
    \end{exampleblock}
\end{frame}
...
\begin{itemize}
    ...
    \pause
    \item Greedy algorithms \textcolor{red}{\textbf{sub-optimal accuracy}}, but \textcolor{teal}{$\boldsymbol{O(2^D)}$} operations {\small \textcolor{purple}{\parencite{breiman1984classification,ID3,c45}}} .
    \pause
    \item Optimal algorithms, \textcolor{teal}{\textbf{optimal accuracy}}, but \textcolor{red}{$\boldsymbol{O((2Np)^D)}$} operations (NP-hard) {\small \textcolor{purple}{\parencite{oct,murtree,pystreed,chaouki2024branchesfastdynamicprogramming,npcomplete}}}.
    \pause
...
\only<1>{\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{schema AAAI.png}
...
\pause
\\
Top-B greedy splits \textcolor{purple}{\cite{topk}}, quantiles, random\dots
\end{frame}
...
\setbeamercovered{transparent}
\begin{frame}{Why generating candidate splits with CART?}
    \pause
...
\onslide<1->{\begin{alertblock}{Challenges {\small \parencite{glanois-survey,lipton,rigourous}}}
\begin{itemize}
    \item<2-> No definition of interpretability.
    \item<3-> Measuring might require humans.
...
\begin{itemize}
    ...
    \item<7-> Less parameters mean more interpretability {\small \textcolor{purple}{\parencite{study-0,study-4}}}.
...
\begin{itemize}
    ...
    \pause
    \item What about (very) big models?
    \pause
...
\begin{frame}{Conclusion: interpretable SDM is a difficult research topic}
    \begin{itemize}
        \pause
        \item Technical challenges: \textcolor{red}{\textbf{partial observability in SDM, NP-hardness}}.
        \\ 
        \pause
        $\rightarrow$ Focus on indirect approaches and/or on POMDP research first.
        \pause
        \item Fundamental challenges: \textcolor{red}{\textbf{no definition}}. 
        \\
        \pause
        $\rightarrow$ Discuss with the community (InterpPol workshop).
        \pause
        \item \textcolor{teal}{\textbf{Decision trees offer good inductive bias for SDM in games or tabular data}}.
    \end{itemize}
    \pause
    \begin{block}{My hope}
...
\begin{frame}{Broader perspectives}
    \begin{itemize}
        ...
        \pause
        \item \textbf{\textcolor{teal}{Teaching:}} Can we use unfolded policies (and interpretability) for teaching?
    \end{itemize}
\end{frame}
